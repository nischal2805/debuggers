...
export default function Landing() {
  ...
  const { user, loading, enterDemoMode, setKnowledgeModel } = useStore()
  ...
  const handleSignIn = async () => {
    try {
      ...
      navigate(userData && !userData.onboarded ? '/onboard' : '/dashboard')
    } catch (err) {
      console.error(err)
    }
  }
  ...
  const handleDemo = () => {
    ...
    const model = buildDefaultKnowledgeModel(token, 'basics')
    setKnowledgeModel(model)
    enterDemoMode(token)
    navigate('/dashboard')
  }
    ...
    <div className="min-h-screen bg-bg-primary flex flex-col overflow-hidden relative">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 text-center">
        ...
        >
          ...
          <p className="font-body text-text-secondary text-lg max-w-md mx-auto mb-10">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={handleDemo}
              className="font-body text-sm text-text-secondary hover:text-text-primary border border-border hover:border-text-secondary/30 px-8 py-3 rounded-lg transition-all duration-200"
            >
              Try without account
            </motion.button>
          </div>
